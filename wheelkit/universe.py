...
# The universe lives in its own directory so a branch checkout that changes
# whether it is tracked cannot delete it from the repository root.
DEFAULT_UNIVERSE_PATH = Path("universe/symbols.txt")
...
def read_symbols_file(path: Path) -> list[str]:
    """Parse a symbols file, ignoring comments and blanks."""
    if not path.exists():
        return []
    return sorted(
        {
...
def build_universe_file(
    ...
    *,
    ...
    """Screen the market and write the universe file, returning its symbols."""
    filters = filters or UniverseFilters()
    report = screen(provider, filters, progress=verbose)
    if not report.kept:
        raise FetchError(
            "Nothing passed the universe screen. Loosen --min-dollar-volume "
            "or widen the price band."
        )
    write_symbols_file(path, report, filters)
    return [entry.symbol for entry in report.kept]
