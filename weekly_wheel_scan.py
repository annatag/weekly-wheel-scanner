...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Scan IBKR live data for 7–14 DTE cash-secured puts.")
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=7497, help="7497 TWS paper; 7496 TWS live; 4002 Gateway paper; 4001 Gateway live")
    p.add_argument("--client-id", type=int, default=21)
    p.add_argument("--symbols", help="Comma-separated tickers. Defaults to built-in liquid universe.")
    p.add_argument("--symbols-file", type=Path, help="Text file with one ticker per line.")
    p.add_argument("--earnings-file", type=Path, default=Path("earnings.csv"), help="CSV: symbol,earnings_date")
...
def get_stock_context(ib: IB, symbol: str) -> tuple[Stock, float, float, float, float] | None:
    ...
    ticker = ib.reqMktData(stock, "", snapshot=True, regulatorySnapshot=False)
    ib.sleep(2)
    spot = first_valid(ticker.marketPrice(), ticker.last, ticker.close, ticker.bid, ticker.ask)
    ...
    return stock, spot, five_day_move, avg_volume, support_20d


def select_chain(ib: IB, stock: Stock):
    chains = ib.reqSecDefOptParams(stock.symbol, "", stock.secType, stock.conId)
    smart = [c for c in chains if c.exchange == "SMART" and c.multiplier == "100"]
...
def score_candidate(c: Candidate, cfg: Config) -> float:
    ...
    quality = min(100, 45 + 12 * math.log10(max(c.avg_stock_volume, 1) / 1_000_000 + 1))

    # 20 premium/risk: reward annualized return, but cap extreme values to avoid lottery-premium bias.
    premium_risk = min(100, max(0, c.annualized_return / 0.30 * 100))

    # 15 liquidity: OI, option volume and spread.
    spread_pct = (c.ask - c.bid) / c.midpoint if c.midpoint > 0 else 1
    liq = min(100, 35 * min(c.open_interest / 1500, 1) + 25 * min(c.option_volume / 300, 1) + 40 * max(0, 1 - spread_pct / cfg.max_spread_pct))

    # 15 IV attractiveness: moderate/high IV earns more, but very high IV is capped.
    iv_score = min(100, max(0, (c.iv - 0.15) / 0.45 * 100))

    # 15 technical support: strike below 20-day low is strongest; slight premium if breakeven is below support.
    cushion_to_support = (c.support_20d - c.breakeven) / c.underlying_price
    technical = min(100, max(0, 60 + cushion_to_support * 1000))

    # 10 market conditions proxy: conservative neutral value; replace with VIX feed if subscribed.
    market = 60

    return round(0.25 * quality + 0.20 * premium_risk + 0.15 * liq + 0.15 * iv_score + 0.15 * technical + 0.10 * market, 1)


def scan_symbol(ib: IB, symbol: str, earnings: dict[str, date], cfg: Config) -> list[Candidate]:
    context = get_stock_context(ib, symbol)
    if context is None:
        return []
    stock, spot, five_day_move, avg_volume, support_20d = context
    ...
    # Keep requests manageable by screening strikes using spot and cash bounds first.
    strikes = sorted(s for s in chain.strikes if min_strike <= s <= max_strike and s < spot)
    ...
    for exp, exp_date, dte in expirations:
        ...
        contracts = [Option(symbol, exp, strike, "P", "SMART", tradingClass=chain.tradingClass) for strike in strikes]
        qualified = ib.qualifyContracts(*contracts)
        if not qualified:
            continue

        # Snapshot market data with OI (101) and option volume (100).
        tickers = [ib.reqMktData(contract, "100,101", snapshot=True, regulatorySnapshot=False) for contract in qualified]
        ...
        for contract, ticker in zip(qualified, tickers):
            ...
            if not (valid_number(bid) and valid_number(ask) and ask >= bid and bid > 0):
                continue
            midpoint = (bid + ask) / 2
            ...
            comp = ticker.modelGreeks or ticker.bidGreeks or ticker.askGreeks or ticker.lastGreeks
            ...
            iv = float(comp.impliedVol) if valid_number(comp.impliedVol) else math.nan
            oi = int(first_valid(getattr(ticker, "putOpenInterest", None), default=0))
            opt_volume = int(first_valid(getattr(ticker, "putVolume", None), ticker.volume, default=0))
            if oi < cfg.min_open_interest or opt_volume < cfg.min_option_volume:
                continue
...
def print_results(rows: Iterable[Candidate]) -> None:
    ...
    if not rows:
        ...
        return
    headers = ["#", "Score", "Ticker", "Exp", "DTE", "Strike", "Bid/Ask", "Delta", "Premium", "Cash", "ROC", "Ann.", "B/E", "5d move", "OI", "Vol"]
    table = []
    for i, c in enumerate(rows, 1):
        table.append([
            i, f"{c.score:.1f}", c.symbol, c.expiration, c.dte, f"${c.strike:.2f}",
            f"${c.bid:.2f}/${c.ask:.2f}", f"{c.delta:.3f}", f"${c.premium:.0f}",
            f"${c.cash_required:,.0f}", f"{c.return_on_cash:.2%}", f"{c.annualized_return:.1%}",
            f"${c.breakeven:.2f}", f"{c.five_day_move:+.2%}", c.open_interest, c.option_volume,
        ])
...
def main() -> int:
    ...
    try:
        ...
        print("Open TWS/IB Gateway, enable API socket clients, and verify host/port.", file=sys.stderr)
        return 2

    print(f"Connected. Scanning {len(symbols)} symbols at {datetime.now().astimezone().isoformat(timespec='seconds')}...")
    all_candidates: list[Candidate] = []
    try:
        for idx, symbol in enumerate(symbols, 1):
            print(f"[{idx}/{len(symbols)}] {symbol}", flush=True)
            try:
                all_candidates.extend(scan_symbol(ib, symbol, earnings, cfg))
            except Exception as exc:
    ...
    ranked = sorted(all_candidates, key=lambda c: (c.score, c.annualized_return), reverse=True)[: cfg.top_n]
    print_results(ranked)
    write_csv(args.output, ranked)
    print(f"\nSaved {len(ranked)} result(s) to {args.output.resolve()}")
    print("Verify bid/ask, Greeks, earnings, and buying power in TWS immediately before trading.")
    return 0
